    ...
    def opt(self, clientID, msg):
		...
		pass
            ...
            if m in ('/stats', '/rank', '/myself', '/me'):
                ...
                for i in bsInternal._getForegroundHostSession().players:
                    if i.getInputDevice().getClientID() == clientID:
                        pbid = str(i.get_account_id())
                    if a == []:
                        bs.screenMessage("Try /perk heal, damage , health")
                    f = open(bs.getEnvironment()['systemScriptsDirectory'] + "/pStats.json", "r")
                    pats = json.loads(f.read())
                    if pbid in pats:
                        kill = pats[pbid]["kills"]
                        death = pats[pbid]["deaths"]
                        k_d = str(int(kill)/int(death))[:3]
                        if a[0] == "damge":
                            if int(kill) >= 100:
                                cmdsetg.damage(pbid)
                            elif int(kill) < 100:
                            ...
                            if float(k_d) >= 1.3:
                                ...
                                bs.screenMessage("You Must Have 1.3 K/D Ratio !")
                        elif a[0] == "health":
		            ...
		            if m == '/quit':
		                ...
		                if m == '/partyname':
		                    ...
		                    if a == []:
		                        ...
		                        bsInternal._chatMessage("4.Big Bomb")
		                    t = int(a[1])
		                    if a[0] == "1":
		                        cmdsetg.sB(t)
		                        commandSuccess = True
